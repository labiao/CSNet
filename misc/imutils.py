...
def random_resize_long(img, min_long, max_long, target_long=None):
    ...
    return pil_rescale(img, scale, 3), target_long


def random_scale(img, scale_range, order, target_scale=None):
    if target_scale is None:
        target_scale = scale_range[0] + random.random() * (scale_range[1] - scale_range[0])

    if isinstance(img, tuple):
        return (pil_rescale(img[0], target_scale, order[0]), pil_rescale(img[1], target_scale, order[1]),
                pil_rescale(img[2], target_scale, order[2]))
    else:
...
def crf_inference_label(img, labels, t=10, n_labels=11, gt_prob=0.7):
    h, w = img.shape[:2]
    ...
    d.addPairwiseBilateral(sxy=50, srgb=5, rgbim=np.ascontiguousarray(np.copy(img)), compat=5)
