...
cls_labels_dict = np.load('dataset/BCD/npy5.npy', allow_pickle=True).item()
...
def load_image_label_list_from_npy_01(img_name_list):
    a = [cls_labels_dict[img_name] for img_name in img_name_list]
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.resize_long:
            imgA, target_long = imutils.random_resize_long(imgA, self.resize_long[0], self.resize_long[1])
            imgB, _ = imutils.random_resize_long(imgB, self.resize_long[0], self.resize_long[1], target_long)

        if self.rescale:
            imgA, target_scale = imutils.random_scale(imgA, scale_range=self.rescale, order=3)
            imgB, _ = imutils.random_scale(imgB, scale_range=self.rescale, order=3, target_scale=target_scale)
        ...
        if self.hor_flip:
            imgA, imgB = imutils.random_lr_flip((imgA, imgB))
            # imgB = imutils.random_lr_flip(imgB)

        if self.crop_size:
            valid_mask = torch.zeros((11, self.crop_size, self.crop_size))
            if self.crop_method == "random":
                imgs, box = imutils.random_crop((imgA, imgB), self.crop_size, (0, 0))
                imgA, imgB = imgs[0], imgs[1]
                valid_mask[:, box[0]:box[1], box[2]:box[3]] = 1
                # imgB, box = imutils.random_crop(imgB, self.crop_size, 0)
            else:
                imgA = imutils.top_left_crop(imgA, self.crop_size, 0)
                imgB = imutils.top_left_crop(imgB, self.crop_size, 0)
                valid_mask = torch.ones((11, self.crop_size, self.crop_size))
        else:
            valid_mask = torch.ones((11, imgA.shape[0], imgA.shape[1]))
        ...
        assert imgA.shape[1] == imgB.shape[1]
        return {'name': name_str, 'imgA': imgA, 'imgB': imgB, 'valid_mask': valid_mask}
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, CAM_root,
        ...
        # self.label_list = load_image_label_list_from_npy_01(self.img_name_list)
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, crop_size, CAM_root,
                 rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False,
                 crop_method='random'):

        self.img_name_list = load_img_name_list(img_name_list_path)
        self.CAM_root = CAM_root
        # 这里的label_dir指的是IRLabel
    ...
    def __getitem__(self, idx):
        ...
        if self.rescale:
            imgA, imgB, label = imutils.random_scale((imgA, imgB, label), scale_range=self.rescale, order=(3, 3, 0))
        ...
        if self.hor_flip:
            imgA, imgB, label = imutils.random_lr_flip((imgA, imgB, label))
            # imgB = imutils.random_lr_flip(imgB)

        if self.crop_method == "random":
            imgA, imgB, label = imutils.random_crop((imgA, imgB, label), self.crop_size, (0, 0, 255))[0]
            # imgB = imutils.random_crop(imgB, self.crop_size, 0)
        elif self.crop_method == 'none':
            imgA, imgB, label = imgA, imgB, label
        else:
            imgA = imutils.top_left_crop(imgA, self.crop_size, 0)
            imgB = imutils.top_left_crop(imgB, self.crop_size, 0)
        ...
        assert imgA.shape[0] == imgB.shape[0]
        assert imgA.shape[1] == imgB.shape[1]
        return {'name': name, 'imgA': imgA, 'imgB': imgB, 'label': label,
                "label_cls": torch.from_numpy(self.label_list[idx])}
