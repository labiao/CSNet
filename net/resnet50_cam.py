...
class Net(nn.Module):
    ...
    def __init__(self, stride=16, n_classes=10):
        ...
        self.newly_added = nn.ModuleList([self.stage5, self.classifier])
...
class Net_CAM(Net):
    ...
    def __init__(self, stride=16,
                 n_classes=10):  # n_class set to 10 to prevent gradient explosion and overfitting,there are actually only two classes.
...
class CAM(Net):
    ...
    def __init__(self, stride=16, n_classes=10):
        # n_class set to 10 to prevent gradient explosion and overfitting,there are actually only two classes.
...
class Class_Predictor(nn.Module):
    ...
    def forward(self, x, label):
        ...
        x = x.reshape(batch_size, self.num_classes, -1)  # [16, 10, 2048]
        mask = label > 0  # [16 10]
        feature_list = [x[i][mask[i]] for i in range(batch_size)]  # 选取那些前景类别的特征
        prediction = [self.classifier(y.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1) for y in feature_list]
        labels = [torch.nonzero(label[i]).squeeze(1) for i in range(label.shape[0])]  # 获取多标签中前景的索引，直接输入至交叉商
        loss = 0
        acc = 0
        num = 0
