...
class Net(nn.Module):
    ...
    def __init__(self, backbone_file, deploy=False, pretrained=True, n_classes=10):
        ...
        self.classifier = nn.Conv2d(2048, 10, 1, bias=False)
        self.num_cls = 2
    ...
    def get_seed(self, norm_cam, label, feature):
        ...
        correlation = F.relu(torch.matmul(feature_s.transpose(2, 1), feature_s), \
                             inplace=True).unsqueeze(1)  # [n,1,h*w,h*w]
...
class CAM(Net):
    ...
    def __init__(self, backbone_file, deploy=False, pretrained=True):
        super(CAM, self).__init__(backbone_file, deploy=deploy, pretrained=pretrained)
