...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(tqdm(infer_data_loader, position=process_id, desc=f'[PID{process_id}]')):
        ...
        # 第二次
        # pathA, pathB = args.amn_cam_out_dir.split('_sec')
        # cam_dict = np.load(os.path.join((pathA + pathB), img_name + '.npy'), allow_pickle=True).item()
        # 第三次
        # cam_dict = np.load(os.path.join(args.amn_cam_out_dir, img_name + '.npy'), allow_pickle=True).item()
...
def run(args):
    dataset = dataloader.VOC12ImageDataset(args.infer_list, CAM_root=args.CAM_root, img_normal=None,
                                                 to_torch=False)
